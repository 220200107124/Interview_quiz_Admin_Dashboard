  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch assigned quizzes - use the assignment endpoint to get assignment data
        const quizRes = await axios.get(`${process.env.REACT_APP_API_URL}/api/assignment/assignments/candidate/${candidateId}`);
        console.log("=== QUIZ RESPONSE DEBUG ===");
        console.log("Quiz response data:", quizRes.data);
        console.log("Type of quiz data:", typeof quizRes.data);
        console.log("Is array:", Array.isArray(quizRes.data));
      ...
      <div className="candidate-quiz-container">
        ...
        {quizzes.length > 0 ? (
          quizzes.map((quizItem, quizIndex) => {
            // Handle both assignment data and quiz data structures
            const questions = quizItem.quizId?.questions || quizItem.quiz?.questions || quizItem.questions || [];
            const quizTitle = quizItem.quizId?.title || quizItem.quiz?.title || quizItem.title || 'Quiz';
            const isSubmitted = submittedQuizzes[quizIndex];

            return (
              <div key={quizIndex} className="question-block">
                <h3>{quizTitle}</h3>
                <p><small>Assignment ID: {quizItem._id}</small></p>
